...
http://ps-tools.data.riakcs.net:8080/riak-data-migrator-0.1.6-bin.tar.gz

After downloading, unzip/untar it, and it's ready to run from its directory.
```bash
tar -xvzf riak-data-migrator-0.1.6-bin.tar.gz
cd riak-data-migrator-0.1.6
java -jar riak-data-migrator-0.1.6.jar [options]
```
...
```
    ...
    The usable binary file is ```riak-data-migrator-0.1.6-bin.tar.gz```
...
```java -jar riak-data-migrator-0.1.6.jar -d -r /var/riak_export -a -h 127.0.0.1 -p 8087 -H 8098```

Load all buckets previously dumped back into Riak:  
```java -jar riak-data-migrator-0.1.6.jar -l -r /var/riak-export -a -h 127.0.0.1 -p 8087 -H 8098```

Dump (the contents of) buckets listed in a line delimited file from a Riak cluster:  
<pre>
java -jar riak-data-migrator-0.1.6.jar -d -f /home/riakadmin/buckets_to_export.txt -r \  
/var/riak-export -c /home/riakadmin/riak_hosts.txt -p 8087 -H 8098
</pre>

Export only the bucket settings from a bucket named "Flights":  
```java -jar riak-data-migrator-0.1.6.jar -d -t -r /var/riak-export -b Flights -h 127.0.0.1 -p 8087 -H 8098```

Load bucket settings for a bucket named "Flights":  
```java -jar riak-data-migrator-0.1.6.jar -l -t -r /var/riak-export -b Flights -h 127.0.0.1 -p 8087 -H 8098```
